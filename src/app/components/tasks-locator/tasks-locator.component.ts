...
export class TasksLocatorComponent extends BaseListComponent<Task> {
  entityListConfig: EntityListConfig<Task> = {
    entityLabel: Configuration.TASK_LOCATOR.labelPlural,
    iconName: Configuration.TASK_LOCATOR.icon,
    font: Configuration.TASK_LOCATOR.font,
    columnDefs: [],
    dataFetchFn: () => {
      const params: HalParam[] = [];
      const param: HalParam = {key: 'type.id', value: magic.taskLocatorTypeId};
      params.push(param);
  ...
  override async postFetchData(): Promise<void> {
    this.entityListConfig.columnDefs = [
      ...
      this.utils.getRouterLinkColumnDef(
        'common.form.name',
        'name',
        `tasksLocator/:id/${magic.taskLocatorTypeId}`,
        {id: 'id'}
      )
    ];
  }
  ...
  override async newData() {
    await this.router.navigate(['tasksLocator', -1, magic.taskLocatorTypeId]);
  }

  override async duplicateItem(id: number) {
    await this.router.navigate(['tasksLocator', -1, magic.taskLocatorTypeId, id]);
  }
